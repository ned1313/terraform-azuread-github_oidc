...
resource "azuread_application_federated_identity_credential" "branches" {
  ...
  display_name          = "${azuread_application.oidc.display_name}-branch-${each.value}"
...
resource "azuread_application_federated_identity_credential" "tags" {
  ...
  application_object_id = azuread_application.oidc.object_id
  display_name          = "${azuread_application.oidc.display_name}-tag-${each.value}"
...
resource "azuread_application_federated_identity_credential" "environments" {
  ...
  application_object_id = azuread_application.oidc.object_id
  display_name          = "${azuread_application.oidc.display_name}-environment-${each.value}"
